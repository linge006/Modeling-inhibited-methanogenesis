...
# times is a 'periodic' time sequence with periods of 24 h;
...
# feedintake is a dataframe containing the feed intake rate per period of 24 h
  ...
  if(length(which(names(Parms) == 'k_NPNi')) == 1) Parms <- c(Parms,k_NPNa=3.5*as.data.frame(t(Parms))$k_NPNi)
  if(length(which(names(Constants) == 'k_NPNi')) == 1) Constants <- c(Constants,k_NPNa=3.5*as.data.frame(t(Constants))$k_NPNi)
  
  #if (output=="FINAL_24h") Constants <- Constants[!names(Constants) %in% 'k_NPNa']
...
C_Ni <- Q_Ni/V_fluid									# Nitrite oncentration
...
U_NP_NPNa <- 1e-3*k_NPNa*Q_Me*Q_NP # 3-NOP conversion to nitrate
U_NP_NPNi <- 1e-3*k_NPNi*Q_Me*Q_NP # 3-NOP conversion to nitrite
...
P_Ni_NPNi <- U_NP_NPNi # Inflow from 3-NOP cleavage

U_Ni_NiEx <- (0.4*k_SoEx+0.4*k_FlEx)*Q_Ni # Fractional outflow 
...
U_Hy_HyMe <- (v_HyMe*Q_Me) / (1+M_Hy_HyMe/C_Hy+(C_NP+C_Ni)/J_NP_HyMe) # Methanogenesis 
...
dNP <- P_NP_InNP - U_NP_NPAb - U_NP_NPEx - U_NP_NPNa - U_NP_NPNi
dNi <- P_Ni_NPNi - U_Ni_NiEx
...
list(c(dFd,dSd,dWr,dNP,dNi,dHe,dMi,dAc,dPr,dBu,dHy,dMe,dNAD,dMt),
     c(D_DMI=D_DMI,P_Fd_InFd=P_Fd_InFd,U_Fd_FdHe=U_Fd_FdHe,U_Fd_FdEx=U_Fd_FdEx,
       P_Sd_InSd=P_Sd_InSd,U_Sd_SdHe=U_Sd_SdHe,U_Sd_SdEx=U_Sd_SdEx,P_Wr_InWr=P_Wr_InWr,U_Wr_WrHe=U_Wr_WrHe,U_Wr_WrEx=U_Wr_WrEx,
       P_NP_InNP=P_NP_InNP,U_NP_NPEx=U_NP_NPEx,C_NP=C_NP,C_Ni=C_Ni,
       ...
       P_NADH_Ac=P_NADH_Ac,U_NADH_AP=U_NADH_AP,U_NADH_Fe=U_NADH_Fe,
       U_Hy_HyMe=U_Hy_HyMe,U_Hy_HyEm=U_Hy_HyEm,P_Mt_HyMe=P_Mt_HyMe+P_Mt_MtLi,P_Mt_MtHg=P_Mt_MtLi,
...
if (output=="CLEAN") {return(out[out$time %in% seq(0,final,10*stpsz),])} # return out while cleaning up the outfile
}
